        ...
        for sources, targets, sources_masks, spacings, sources_landmarks, targets_landmarks, sources_landmarks_md, targets_landmarks_md  in training_dataloader:
            for i in range(len(sources)):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    cost = cost_function(transformed_source, target, device=device, **cost_function_params)
                    reg = alpha * reg_function(df_nonrigid, device=device, **reg_function_params)
                    penalty_function_params['mask'] = source_mask
                    penalty = 0.01*penalty_function(df_nonrigid, device=device, **penalty_function_params)
