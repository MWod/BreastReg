...
from networks import nonrigid_network_multilevel as nn
...
def training(training_params):
    ...
    penalty_function = rg.volume_penalty_tc
    ...
    for epoch in range(num_epochs):
        ...
        for sources, targets, sources_masks, spacings, sources_landmarks, targets_landmarks, sources_landmarks_md, targets_landmarks_md  in training_dataloader:
            for i in range(len(sources)):
                ...
                target_landmarks_md = targets_landmarks_md[i]
                spacing = spacings[i]
                source_mask = sources_masks[i]
                source = source.to(device).view(1, 1, source.size(0), source.size(1), source.size(2))
                target = target.to(device).view(1, 1, target.size(0), target.size(1), target.size(2))
                source_mask = source_mask.to(device).view(1, 1, source_mask.size(0), source_mask.size(1), source_mask.size(2))

                source = source + tc.rand(source.size()).to(device)*0.000001
                target = target + tc.rand(target.size()).to(device)*0.000001

                source_pyramid = utc.create_pyramid(source, 3, device=device)
                target_pyramid = utc.create_pyramid(target, 3, device=device)
                source_mask_pyramid = utc.create_pyramid(source_mask, 3, device=device)

                with torch.set_grad_enabled(True):
                    df_nonrigid = model(source_pyramid, target_pyramid)
                    for k in range(len(df_nonrigid)):
                        transformed_source = utc.warp_tensor(source_pyramid[k], df_nonrigid[k], device=device)
                        cost = cost_function(transformed_source, target_pyramid[k], device=device, **cost_function_params)
                        reg = alpha * reg_function(df_nonrigid[k], device=device, **reg_function_params)
                        penalty_function_params['mask'] = source_mask_pyramid[k]
                        penalty = 0.01*penalty_function(df_nonrigid[k], device=device, **penalty_function_params)
                        if k == 0:
                            loss = cost + reg + penalty
                        else:
                            loss = loss + cost + reg + penalty
                    loss = loss / len(df_nonrigid)
                    loss.backward()
                    optimizer.step()
                optimizer.zero_grad()

                df_np_nonrigid = utc.tc_df_to_np_df(df_nonrigid[-1])
                ...
                train_running_reg += reg.item()
                train_running_vr += penalty.item()
        ...
        print("Train VR: ", train_running_vr / training_size)
        ...
        train_vr_history.append(train_running_vr / training_size)
        ...
        for sources, targets, sources_masks, spacings, sources_landmarks, targets_landmarks, sources_landmarks_md, targets_landmarks_md  in validation_dataloader:
            for i in range(len(sources)):
                with torch.set_grad_enabled(False):
                    ...
                    transformed_source = utc.warp_tensor(source, df_nonrigid[-1], device=device)
                    loss = cost_function(transformed_source, target, device=device, **cost_function_params)
                    reg = alpha * reg_function(df_nonrigid[-1], device=device, **reg_function_params)
                    penalty_function_params['mask'] = source_mask
                    penalty = 0.01*penalty_function(df_nonrigid[k], device=device, **penalty_function_params)
                    ...
                    val_running_reg += reg.item()
                    val_running_vr += penalty.item()
        ...
        print("Val VR: ", val_running_vr / validation_size)
        ...
        val_vr_history.append(val_running_vr / validation_size)
    ...
    plt.figure()
    plt.plot(train_vr_history, "r-")
    plt.plot(val_vr_history, "b-")
    plt.grid(True)
    plt.xlabel("Epoch")
    plt.ylabel("VR")
    plt.legend(["Train", "Validation"])
    plt.savefig(os.path.join(figures_path, model_name + "_vr.png"), bbox_inches = 'tight', pad_inches = 0)
    ...
    for sources, targets, sources_masks, spacings, sources_landmarks, targets_landmarks, sources_landmarks_md, targets_landmarks_md  in dataloader:
        with torch.set_grad_enabled(False):
            for i in range(len(sources)):
                ...
                print("Reg: ", reg_function(df_nonrigid[-1], device=device))
...
def run():
    ...
    training_params['initial_path'] = None # TO DO
